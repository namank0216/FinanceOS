...
st.set_page_config(page_title="Stock Discovery", layout="wide")
st.title("🔍 Stock Discovery & Analysis")
st.caption("Screen the universe → pick a candidate → analyze it. Everything on one page.")

explainers.help_box(st, "How this page works (in plain English)", """
**One-page flow.**
1. Pick the universe (default: **Nasdaq 100**) and the filter rule.
2. Click **▶ Run scan** — the system runs stage analysis + factor scoring across every stock.
3. The results table shows ranked candidates that meet your filter.
4. Pick any stock from the dropdown below the table (or type a ticker manually) — the full
   **Stage Engine detail** opens below: stage, 5-factor composite, narrative, chart, factor breakdown.

**The institutional decision rule baked in as default:**
> **Stage 2 + composite ≥ +50** = high-conviction long candidate.
> **Stage 4 or composite ≤ -25** = avoid regardless of how good the story sounds.

This page replaces the former separate Screener and Stage Engine pages.
""")
...
if "scan_full" in st.session_state:
    ...
    if filtered.empty:
        ...
        if screen_narrative:
            ...
            ai_summary.render_ai_button(st, ai_ctx, key="discovery_scan")
...
ai_summary.render_ai_button(st, ai_ctx, key=f"discovery_stock_{ticker}")
